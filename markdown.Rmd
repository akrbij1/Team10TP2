...
When exploring the other solutions posted in Kaggle, we found that the majority of the submissions had a very similar implementation of XGBoost. The main differences between the submissions relied in the feature creation based on the data provided. Data was given on multiple CSV files which allowed for a lot of freedom in the feature creation. This differences in features did not permit us to try many different models since it would require extensive data preparation.

There was one major division between the models. Some tried to create models with the capacity to determine if a product was going to be reordered or not. The rest of the submissions are non-supervised models such as K-Means, which are intended to identify market segments that could later be used for predictive purposes. We believed that a predicion of product reorders had a higher business value since it has a very practical implementation on daily operations. 


One of the reasons why XGBoost is so widely used for this problem is because it is the only algorithm that has the capacity to run with such a big data set. Looking at the top submissions we can quickly see that all of the top performing models are run with XGBoost. 
...
cat('Total time to compile markdown:', total_time, "seconds.")
```
